...
//////////////////////////////////////////////////////////////////////////
  ...
  await page.locator('div.saleitembrowser_SaleItemBrowserHeaderContainer__MBp9.Panel.Focusable > div > div:nth-child(2)').click();
...
await test.step('Select game with maximum discount or maximum price', async () => {
  ...
  if (gamesWithDiscount.length > 0) {
    ...
    const gameLinkSelector = "//div[contains(@class, 'StoreSalePriceWidgetContainer')]/div[contains(text(),'" + MaxDiscountVariable + "')]/ancestor::div[@class='ImpressionTrackedElement']//div[contains(@class, 'StoreSaleWidgetTitle')]/parent::a";

    // Находим ссылку на выбранную игру
    const selectedGameLink = await gamesWithDiscount[maxDiscountIndex].$(gameLinkSelector);

    if (selectedGameLink) {
        // Кликаем на ссылку и ожидаем перехода
        await selectedGameLink.click();
        await page.waitForTimeout(2000);
        await expect(page).toHaveURL("https://store.steampowered.com/app/538030/Xenonauts_2/");
    } else {
        // Обработка ошибки
